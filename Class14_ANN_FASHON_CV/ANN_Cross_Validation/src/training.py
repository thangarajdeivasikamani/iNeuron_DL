...
def training(config_path):
    ...
    n_splits = config["params"]["n_splits"]
    seed = config["params"]["seeds"]
    # Get the dataset
    ...
    cvscores = []
    # Split the X and Y data randomly using K Fold, 10 times will repeat
    for train, test in kfold.split(X, Y):
        ...
        #Batch size & EPOC - Batch Gradient
        ...
        logging.info(f"History:",history.params)
        #Create the plot
        history_dataframe = pd.DataFrame(history.history)
        # save the history into csv file:
        csv_file_name = os.path.join(log_dir_path,"Histroy.csv")
        #print(csv_file_name)
    ...
    history = ckpt_model.fit(X[train], Y[train], epochs=EPOCHS,
